# ASIGA
The ASIGA (Acoustic Scattering using IsoGeometric Analysis) toolbox provides a framework for simulating acoustic scattering problems using isogeometric analysis (IGA).

The toolbox contains a range of different methods for solving acoustic scattering problems setting the groundwork for research in this area. A range of geometries are included as well, in particular the BeTSSi submarine (illustrated in the figure below). 
![Missing BeTSSi submarine nearfield plot](https://github.com/Zetison/ASIGA/blob/master/miscellaneous/BCA_p_tot.png?raw=true)
The main focus is computing the so-called target strength (TS) for a given geometry (as illustrated in the next figure).
...
The underlying/governing equations and results produced by this toolbox can be found in \[[1](https://www.sciencedirect.com/science/article/abs/pii/S004578251830094X), [2](https://www.sciencedirect.com/science/article/pii/S0045782519305559)\].

## Installation
Get the toolbox from GitHub (the [e3Dss repository](https://github.com/Zetison/e3Dss) is needed for exact solutions and export_fig is needed for improved graphics for publications):
```
...
The main emphasis is acoustic scattering using plane-wave simulations (applyLoad = 'planeWave') for which these cases are implemented:
- Bistatic scattering                                                (scatteringCase =    'BI')
- Monostatic scattering                                              (scatteringCase =    'MS')
- Frequency sweep                                                    (scatteringCase = 'Sweep')


These methods are implemented                                        (with available formulations):
...
- IGA using best approximation                                         (method = 'BA'). Only available when an analytic solution exists
...
- IGA using the boundary element method                                (method = 'BEM')
	...
	- Hyper-singular boundary integral equation using the collocation method (formulation = 'CHBIE')
	- Burton-Miller formulation using the collocation method           (formulation = 'CBM')
	- Three conventional boundary regularized integral equations using the collocation method
	                                                                   (formulation = 'CCBIE1', formulation = 'CCBIE2' and formulation = 'CCBIE3')
...
## Boundary Conditions
These boundary conditions are implemented:
...
where the latter option is used to simulate manufactured solutions
...
loopParameters                  = {'M'};       % Parameter study of ARR to be investigated
runTasksInParallel              = false;       % Run tasks in parallel
computeCondNumber               = false;       % Compute the condition number of the global matrix
N_max                           = inf;         % Number of terms in analytic solution for scattering on a spherical shell
progressBars                    = false;       % Show progress bars for building system matrices
subFolderName                   = '';          % Subfolder wherein results are stored, in the <folderName> parent folder
Conjugated
%% Storage settings
...
%% Mesh settings
M                               = 1;           % Mesh number
initMeshFactXi                  = 1;           % Initial number of knots in the xi direction
initMeshFactEta                 = 1;           % Initial number of knots in the eta direction
initMeshFactZeta                = 1;           % Initial number of knots in the zeta direction
extraGP                         = 0;           % Extra quadrature points
parm                            = 1;           % Toggle different parameterizations of a geometric model

%% Settings for pre-plotting (geometry and mesh visualization)
prePlot.plot2Dgeometry          = false;       % Plot cross-section of mesh and geometry
prePlot.plot3Dgeometry          = false;       % Plot visu-conjugated conjugation of mesh and geometry in 3D
prePlot.storeFig                = false;       % Store pre-plotted figure
prePlot.abortAfterPlotting      = false;       % Abort simulation after pre-plotting
prePlot.view                    = getView;     % Set angle of view [azimuth,elevation]
...
prePlot.colorControlPolygon     = 'red';       % Controls polygon line color
prePlot.markerEdgeColor         = 'black';     % Controls the edge-marker color of polygons
prePlot.markerColor             = 'black';     % Controls the marker color of polygons
prePlot.LineWidth               = 0.5;         % Line width
prePlot.elementBasedSamples     = false;       % Uses distance rather than elements for sampling if true
prePlot.samplingDistance        = NaN;         % Adjusts sampling distance if using elementBasedSamples
prePlot.title                   = '';          % Sets figure title
prePlot.axis                    = 'off';       % Sets axis() property
prePlot.xlabel                  = 'x';         % Sets x-axis label
prePlot.ylabel                  = 'y';         % Sets y-axis label
prePlot.zlabel                  = 'z';         % Sets z-axis label
...
plotFarField                    = true;        % Plots the near-field instead if false
...
r       = 1;                                   % Radii for near-field evaluation. Assumes plotFarField to be true by default
...
para.plotMesh                   = true;	       % Create additional ParaView files to visualize the IGA mesh
para.plotP_inc                  = true;
...
agpBEM                          = 1.4;         % Parameter for adaptive Gauss point integration around singularities for BEM formulations
quadMethodBEM   	        = 'Adaptive2'; % In BEM: Quadrature method for handling weakly singular integral equations
...
N                 	        = 3;           % Number of basis functions in the radial direction for the IEM
IEbasis	                        = 'Chebyshev';

%% Settings for the MFS (method of fundamental solutions)
...
k_ROM    	                = 3;           % Points at which derivatives are computed
useDGP                          = true;        % Use a derivative-based Galerkin Projection (instead of interpolation techniques)
```

## Performing a Study
A study may be performed over several sets of parameters. For example, using the following commands in the task script:
```Matlab
M                               = [1,2,5];
degree = [2,5];
...
## Disclaimer
Certain combinations of parameters/methods are not implemented, or may be erroneous. Please report any bugs to jonvegard.venas@sintef.no
