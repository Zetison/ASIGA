function nurbs = getWineGlassData()
...
nurbs = elevateNURBSdegree(nurbs,[0 0 1]);
Xi = nurbs.knots{1};
Eta = nurbs.knots{2};
Zeta = nurbs.knots{3};

controlPts = nurbs.coeffs;
% create smooth edge at top
controlPts(3,:,end,2) = controlPts(3,:,end,5) + t1/2;
controlPts(3,:,end,3) = controlPts(3,:,end,5) + t1/2;
controlPts(3,:,end,4) = controlPts(3,:,end,5) + t1/2;

controlPts(4,:,end,2) = controlPts(4,:,end,2)*1/sqrt(2);
controlPts(4,:,end,4) = controlPts(4,:,end,4)*1/sqrt(2);

controlPts(1:2,:,end,2) = controlPts(1:2,:,end,1);
controlPts(1:2,:,end,4) = controlPts(1:2,:,end,5);

% create smooth edge at bottom
controlPts(1:2,:,1,2) = controlPts(1:2,:,1,2) + sign(controlPts(1:2,:,1,2))*t2/2;
controlPts(1:2,:,1,3) = controlPts(1:2,:,1,3) + sign(controlPts(1:2,:,1,3))*t2/2;
controlPts(1:2,:,1,4) = controlPts(1:2,:,1,4) + sign(controlPts(1:2,:,1,4))*t2/2;

controlPts(4,:,1,2) = controlPts(4,:,1,2)*1/sqrt(2);
controlPts(4,:,1,4) = controlPts(4,:,1,4)*1/sqrt(2);

controlPts(3,:,1,2) = controlPts(3,:,1,1);
controlPts(3,:,1,4) = controlPts(3,:,1,5);



nurbs = createNURBSobject(controlPts,{Xi, Eta, Zeta});
% 
% controlPts(1,:,end,2) = controlPts(3,:,1,2);
