...
%            1,       w,               1]; 
% RR = [R2+y,R1+y];
x1 = t*(R2-R1)/sqrt(L^2+(R2-R1)^2);
y1 = t*L/sqrt(L^2+(R2-R1)^2);
a = -(R2-R1)/L;
% theta = acos(x1/t);
% w1 = cos(theta/2);
w1 = 1/sqrt(2);
coeffs1 = [x1,    R1+t,         R1+t;
           R1+y1, a*(R1+t-x1)+R1+y1, 0;
           1,    w1,            1];
arc1 = createNURBSobject(coeffs1,[0,0,0,1,1,1]);
x2 = t*(R1-R2)/sqrt(L^2+(R1-R2)^2);
y2 = t*L/sqrt(L^2+(R1-R2)^2);
a2 = -(R1-R2)/L;
% theta = acos(x2/t);
% w2 = cos(theta/2);
w2 = 1/sqrt(2);
coeffs2 = [-L-R2-t, -L-R2-t,         -L-x2;
           0,       a2*(R2+t-x2)+R2+y2, R2+y2;
           1,       w2,               1]; 
RR = [R2+y2,R1+y1];
