...
for i = 1:mesh1
    fun = @(theta) findArcLength(R_a,f,theta_prev,theta) - totArcLength1/(mesh1+1);
    theta = bisection(fun, theta_prev, pi, 100, 1e-12);
    
    phi = 0;
    x_vec = [R_a*cos(theta);
             sqrt(R_a^2-f^2)*sin(theta)*cos(phi);
             sqrt(R_a^2-f^2)*sin(theta)*sin(phi)];
    fun = @(eta) norm(x_vec - evaluateNURBS(water, [0, eta, 1]));
...
for i = 1:mesh3
    fun = @(theta) findArcLength(R_a,f,theta_prev,theta) - totArcLength2/(mesh3+1);
