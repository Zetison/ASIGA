...
% TS = zeros(size(task.alpha));
% 
% p_ref = complex(zeros(size(task.alpha)));
% abs_p_ref = zeros(size(task.alpha));
% TS_ref = zeros(size(task.alpha));
% 
% error_pAbs = zeros(size(task.alpha));
% error_p = zeros(size(task.alpha));
% energyError = 0.0;
% L2Error = 0.0;
% H1Error = 0.0;
% surfaceError = 0.0;
...
%                             [A, FF] = buildRBEMmatrix(varCol);  
    ...
    %                     [L_A,U_A] = ilu(A,struct('type','ilutp', 'droptol', 1e-4));
            ...
            %         [UU,~,~,it1,rv1] = cgs(A,FF,1e-20,1000,L_A,U_A);
                ...
                case 'LU'
                    if useROM
%                         fprintf(['\n%-' num2str(stringShift) 's'], 'Creating LU factorization ... ')
