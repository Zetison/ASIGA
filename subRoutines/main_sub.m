        ...
        %             [A_K, A_M] = buildGlobalMatrices(varCol, options);
            ...
            case 'BA'
                ...
                if useInnerFluidDomain
                    ...
                    [A_fluid_i, FF_fluid_i, varCol_fluid_i] = bestApproximationVec(varCol_fluid_i);
                ...
                elseif useSolidDomain && ~useInnerFluidDomain
                    ...
                    A((varCol_solid.noDofs+1):noDofs_tot,(varCol_solid.noDofs+1):noDofs_tot) = A_fluid_o; 
                    shift = 0;
                    FF(1:varCol_solid.noDofs) = FF_solid;
                    ...
                    A((varCol_fluid_i.noDofs+varCol_solid.noDofs+1):noDofs_tot,(varCol_fluid_i.noDofs+varCol_solid.noDofs+1):noDofs_tot) = A_fluid_o;
                    shift = varCol_fluid_i.noDofs;
                    FF(1:varCol_fluid_i.noDofs) = FF_fluid_i;
                    FF((varCol_fluid_i.noDofs+1):(varCol_fluid_i.noDofs+varCol_solid.noDofs)) = FF_solid;
                    FF((varCol_fluid_i.noDofs+varCol_solid.noDofs+1):noDofs_tot) = FF_fluid_o;
                end
        ...
        case 'KDT'
            switch coreMethod
                case 'linear_FEM'
                    ...
                    varCol.h_max = max([norm2(P(tri(:,1),:)-P(tri(:,2),:)); 
                                        norm2(P(tri(:,1),:)-P(tri(:,3),:)); 
                                        norm2(P(tri(:,2),:)-P(tri(:,3),:))]);
                    varCol.dofs = size(unique(tri,'rows','stable'),1);
...
% %                     
%                     export_fig(['../../graphics/sphericalShell/trianglesParm2_' num2str(varCol.noElems)], '-png', '-transparent', '-r300')
%                     
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %             extraZetaPts = extraXiPts; % .. per element
                ...
                if boundaryMethod
                    if strcmp(method, 'KDT')
                        U_fluid_o = zeros(varCol.noDofs,1);
                    end
...
varCol.tot_time = toc(t_start);

fprintf('\nTotal time spent on task: %12f', varCol.tot_time)  
...
if storeFullVarCol || useROM
    ...
    if ~strcmp(method,'RT')
        ...
        if isfield(varCol,'tau')
            varColTemp.tau = varCol.tau;
        end
