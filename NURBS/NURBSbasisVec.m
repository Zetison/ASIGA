function vargout = NURBSbasisVec(I,xi,degree,knots,weights,n)
...
dd = ones(d_p);
order = degree+1;
w_i = zeros([npts,order]);
for i = 1:d_p
    dd(i,i) = 2;
    temp = ones(d_p,1);
    temp(i) = degree(i)+1;
    w_i = w_i + reshape(I(:,i)-degree(i)+(1:degree(i)+1),[npts,temp]);
end
for i = 1:size(xi,1)
    w(i,:) = weights((1:degree(1)+1)+I(i,1),(1:degree(2)+1)+I(i,2),(1:degree(3)+1)+I(i,3));
end
counter = 1;
switch d_p
    case 1
    ...
    case 2
        for k2 = 1:degree(2)+1
            for k1 = 1:degree(1)+1    
                ...
                W = W + B{1}(:,k1,1).*B{2}(:,k2,1)*w;
                if n > 0
                    for i = 1:d_p
                        dW{i}(:,:,1) = dW{i}(:,:,1) + B{1}(:,k1,dd(i,1)).*B{2}(:,k2,dd(i,2))*w;
                    end
                end
                counter = counter + 1;
            end
        ...
        for k3 = 1:degree(3)+1
            for k2 = 1:degree(2)+1
                for k1 = 1:degree(1)+1   
                    ...
                    W = W + B{1}(:,k1,1).*B{2}(:,k2,1).*B{3}(:,k3,1)*w;
                    if n > 0
                        for i = 1:d_p
                            dW{i}(:,:,1) = dW{i}(:,:,1) + B{1}(:,k1,dd(i,1)).*B{2}(:,k2,dd(i,2)).*B{3}(:,k3,dd(i,3))*w;
                        end
                    end
                    counter = counter + 1;
                end
