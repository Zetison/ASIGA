function F = applyNeumann3Dfluid(varCol, surfaces, funcs, coordinateSystem)
...
if surfaces(3)
    g_eta0 = funcs{counter};
    counter = counter + 1;
end
if surfaces(4)
    g_eta1 = funcs{counter};
    counter = counter + 1;
end
if surfaces(5)
    g_zeta0 = funcs{counter};
    counter = counter + 1;
end
if surfaces(6)
    g_zeta1 = funcs{counter};
end
            ...
            switch coordinateSystem
                ...
                case 4
                    k_vec = -[k*cos(beta_s)*sin(alpha_s);
                              k*sin(beta_s)*ones(1,length(alpha_s));
                              k*cos(beta_s)*cos(alpha_s)];

                    P_inc = P_0*exp(-1i*dot2(k_vec,x_0)).*exp(1i*dot2(k_vec,x_vec));
                    
                    deriv5 = -1i*dot2(k_vec,n).*P_inc;
            end
