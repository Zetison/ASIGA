function nurbsPatches = degenerateIGAtoFEM_Grev(nurbsPatches,coreMethod)

if ~iscell(nurbsPatches)
    nurbsPatches = {nurbsPatches};
end
if strcmp(coreMethod,'SEM')
    nurbsPatches = explodeNURBS(nurbsPatches,'xi');
    if strcmp(nurbsPatches{1}.type,'3Dsurface') || strcmp(nurbsPatches{1}.type,'3Dvolume')
        nurbsPatches = explodeNURBS(nurbsPatches,'eta');
    end
    if strcmp(nurbsPatches{1}.type,'3Dvolume')
        nurbsPatches = explodeNURBS(nurbsPatches,'zeta');
    end
end
        ...
        case '3Dvolume'
            switch coreMethod
                case 'SEM'
                    ...
                    [tXi,rhoXi] = GLLpoints(Nxi);
                    if Neta == Nxi
                        tEta = tXi;
                        rhoEta = rhoXi;
                    else
                        [tEta,rhoEta] = GLLpoints(Neta);                        
                    end
                    if Nzeta == Nxi
                        tZeta = tXi;
                        rhoZeta = rhoXi;
                    ...
                    temp = evaluateNURBS_2ndDeriv(nurbs, [reshape(xi,noDofs,1),reshape(eta,noDofs,1),reshape(zeta,noDofs,1)]);
%                     parfor l = 1:Nzeta
