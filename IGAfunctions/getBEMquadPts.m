function [BIE, integrals, FF_temp, sctr_y, noGp, collocationPointIsInElement, pD,y,R_y,Phi_kTemp,dPhi_kTemp,fact_y] = getBEMquadPts(e_y,Q2D_2,W2D_2,Q,W,integrals,FF_temp,...
...
if collocationPointIsInElement % use polar integration
    switch quadMethodBEM
        case 'Simpson'
            ...
            J_5 = zeros(4*noGp,1);
            counter = 0;
            for i = 1:4 %{'East', 'North', 'West', 'South'}
                switch i
                    case 1 %'East'
                        if abs(xi_x_t - 1) < Eps
                            continue
        ...
        case 'Adaptive2'
            ...
            xi_x_t = xi_x_tArr(idx);
            eta_x_t = eta_x_tArr(idx);
            xi_x = parent2ParametricSpace(Xi_e_y, xi_x_t);
            eta_x = parent2ParametricSpace(Eta_e_y, eta_x_t);
            ...
            for i = 1:4
                switch i
                    case 1
                        if abs(xi_x - Xi_e_y(2)) < Eps
                            n_div(i) = 0;
                        end
                    case 2
                        if abs(eta_x - Eta_e_y(2)) < Eps
                            n_div(i) = 0;
                        end
                    case 3
                        if abs(xi_x - Xi_e_y(1)) < Eps
                            n_div(i) = 0;
                        end
                    case 4
                        if abs(eta_x - Eta_e_y(1)) < Eps
                            n_div(i) = 0;
                        end
                end
            end
            ...
            rho_arr  = linspace(0,1,n_div_r+1);
            xi_v = [Xi_e_y(2),Xi_e_y(2),Xi_e_y(1),Xi_e_y(1),Xi_e_y(2)]; % xi at vertices
            eta_v = [Eta_e_y(1),Eta_e_y(2),Eta_e_y(2),Eta_e_y(1),Eta_e_y(1)]; % xi at vertices
            ...
            for i = 1:4
                ...
                n_div_theta = n_div(i);
                rho = zeros(no_gpSource^2,n_div_r*n_div_theta);
                theta = zeros(no_gpSource^2,n_div_r*n_div_theta);
                thetas_arr  = linspace(0,1,n_div_theta+1);
                counter = 1;


                for i_theta = 1:n_div_theta
                    theta_sub2 = thetas_arr(i_theta:i_theta+1);
                    for i_rho = 1:n_div_r
                        rho_sub = rho_arr(i_rho:i_rho+1);
                        rho(:,counter) = parent2ParametricSpace(rho_sub, Q_r);
                        theta(:,counter) = parent2ParametricSpace(theta_sub2, Q_theta);
                        counter = counter + 1;
                    end
                end
                noSubElementsTri = n_div_r*n_div_theta;
                rho = reshape(rho,noSubElementsTri*no_gpSource^2,1);
                theta = reshape(theta,noSubElementsTri*no_gpSource^2,1);
                                             
                indices = counter2:counter2+numel(rho)-1;
                xi_y(indices) = xi_x + rho.*(xi_v(i)-xi_x+(xi_v(i+1)-xi_v(i))*theta);
                eta_y(indices) = eta_x + rho.*(eta_v(i)-eta_x+(eta_v(i+1)-eta_v(i))*theta);
                J_3(indices) = rho.*(  (xi_v(i)-xi_x+(xi_v(i+1)-xi_v(i))*theta)*(eta_v(i+1)-eta_v(i)) ...
                                       - (eta_v(i)-eta_x+(eta_v(i+1)-eta_v(i))*theta)*(xi_v(i+1)-xi_v(i)));
                J_4(indices) = 0.25/n_div_r/n_div_theta;                
                J_5(indices) = 1;
                
                counter2 = counter2 + numel(rho);
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                if pD.plotGP
                    for i_theta = 1:n_div_theta
                        theta_sub = thetas_arr(i_theta:i_theta+1);
                        rho2 = linspace(Eps,1-Eps,100).';
                        for j = 1:2
                            theta2 = theta_sub(j)+Eps*(-1)^(j+1);
                    ...
                    for i_rho = 1:n_div_r-1
                        rho_sub = rho_arr(i_rho:i_rho+1);
                        rho2 = rho_sub(2);
                        theta2 = linspace(Eps,1-Eps,100).';
...
%             n_div   = round(sqrt(numel(W2D_2)/4)/(p_max+1)/2)*ones(1,4);
        ...
        %                 J_2 = 0.25*(Xi_e(2)-Xi_e(1))*(Eta_e(2)-Eta_e(1))/n_div^2;
                ...
                for i_theta = 1:n_div_theta
                    theta_sub = thetas{i}(i_theta:i_theta+1);
        ...
        %                 theta = parent2ParametricSpace(thetaRange,Q2D_2(:,1));
                ...
                temp = repmat(thetas{i}(2:end)-thetas{i}(1:end-1),n_div_r*no_gpSource^2,1);
                ...
                if pD.plotGP
                    ...
                    for i_rho_t = 1:n_div_r-1
                        rho_t_sub = rho_t_arr(i_rho_t:i_rho_t+1);
                        rho_t2 = rho_t_sub(2);
                        theta2 = linspace(thetas{i}(1)+Eps,thetas{i}(end)-Eps,100).';
                        switch areas{i}
                            case 'East'
                                rho_hat2 = ( 1 - xi_x_t)./cos(theta2);
                            case 'North'
...
if n_div < 2 % || recursionLevel > 200
    ...
    for jj = 1:2
        for ii = 1:2
            corner_i = indexMap(ii:ii+1,jj:jj+1);
            h = max([norm(y_temp(corner_i(1),:)-y_temp(corner_i(4),:)), norm(y_temp(corner_i(2),:)-y_temp(corner_i(3),:))]);
            x_5 = y_temp(cntr,:);
            Xi_eSub = Xi_eSubArr(ii:ii+1);
            Eta_eSub = Eta_eSubArr(jj:jj+1);
