    ...
    switch quadMethodBEM
        ...
        case 'Adaptive2'
            ...
            for area = 1:4
                ...
                thetas2_arr  = linspace(0,1,n_div_theta+1);
                counter = 1;


                for i_theta = 1:n_div_theta
                    theta_sub2 = thetas2_arr(i_theta:i_theta+1);
                ...
                xi_t  = xi_x_t + rho_t.*(xi_1_t-xi_x_t+(xi_2_t-xi_1_t)*theta);
                eta_t = eta_x_t + rho_t.*(eta_1_t-eta_x_t+(eta_2_t-eta_1_t)*theta);
                
                indices = counter2:counter2+numel(xi_t)-1;
                xi_y(indices) = parent2ParametricSpace(Xi_e_y, xi_t);
                eta_y(indices) = parent2ParametricSpace(Eta_e_y, eta_t);
                J_3(indices) = rho_t.*(  (xi_1_t-xi_x_t+(xi_2_t-xi_1_t)*theta)*(eta_2_t-eta_1_t) ...
                                       - (eta_1_t-eta_x_t+(eta_2_t-eta_1_t)*theta)*(xi_2_t-xi_1_t));
                J_4(indices) = 1/n_div_r/n_div_theta;                
                J_5(indices) = 0.25;
                ...
                if pD.plotGP
                    for i_theta = 1:n_div_theta
                        theta_sub = thetas2_arr(i_theta:i_theta+1);
                        rho_t2 = linspace(Eps,1-Eps,100).';
                        for ii = 1:2
                            ...
                            xi_t2  = xi_x_t + rho_t2.*(xi_1_t-xi_x_t+(xi_2_t-xi_1_t)*theta2);
                            eta_t2 = eta_x_t + rho_t2.*(eta_1_t-eta_x_t+(eta_2_t-eta_1_t)*theta2);
