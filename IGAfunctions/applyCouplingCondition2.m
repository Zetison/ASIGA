...
for j = 1:n_eta
    for i = 1:n_xi
        fluidNodes(counter) = (n_eta*n_xi)*(n_zeta-1) + n_xi*(j-1) + i;
        counter = counter + 1;
    end
end
...
% for e = 1:solidNoElemsXiEta
    ...
    for i = 1:d
        solidSctrXiEtadD(i:d:d*n_en) = d*(solidSctrXiEta-1)+i + shift;
    end
...
[spIdx2,~,IuniqueIdx2] = unique([spIdxRow2, spIdxCol2],'rows');
