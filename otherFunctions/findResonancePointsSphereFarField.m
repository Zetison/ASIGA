function cps = findResonancePointsSphereFarField(dmx, R, etaShift, elementList, xiE, etaE)
cps.cp_a = zeros(2,2);
cps.cp_b = zeros(2,2);
cps.cp_c = zeros(2,2);
cps.cp_d = zeros(2,2);
    ...
    for i = 1:2
        ...
        if i == 1
            cps(e).cp_a = [etaE(1), etaE(2);
                            0, 0];
            if (etaE(1)+Eps < y_res) && (y_res < etaE(2)-Eps)
                cps(e).cp_a = [etaE(1), y_res, etaE(2);
                               0, r, 0];
            elseif abs(y_res - etaE(2)) < Eps
                cps(e).cp_a(:,2) = [etaE(2); r];
            elseif abs(y_res - etaE(1)) < Eps
    ...
    for i = 1:2
        ...
        if i == 1
            cps(e).cp_c = [xiE(1), xiE(2);
                            0, 0];
            if (xiE(1)+Eps < x_res) && (x_res < xiE(2)-Eps)
                cps(e).cp_c = [xiE(1), x_res, xiE(2);
                               0, r, 0];
            elseif abs(x_res - xiE(1)) < Eps
                cps(e).cp_c(:,1) = [xiE(1); r];
            elseif abs(x_res - xiE(2)) < Eps
