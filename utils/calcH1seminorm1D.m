function Error = calcH1seminorm1D(varCol, U)
...
for e = 1:noElems
    ...
    for gp = 1:size(W1D,1)
        ...
        X = R_fun*pts;
        du = dudx(X);
        Error = Error + abs(du-du_h)^2 * abs(J_1) * J_2 * wt;
        normalization = normalization + abs(du)^2 * abs(J_1) * J_2 * wt;
    end
end
